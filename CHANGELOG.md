...
### Refactoring
- [Tests] Replace 6 `TODO` stubs in `testLocalization()` with neutral returns (`emptyList()`, `null`, `false`, minimal anonymous objects) — prevents silent `NotImplementedError` if future tests invoke those paths (TASK-V)
