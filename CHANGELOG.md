...
- [Action] Add `MoveI18nKeyHandlerTest` — exercises the real `execute` write path: single-locale move + source deletion, multi-locale move, missing-target-file creation (no data loss), collision overwrite, code-reference rewrite (explicit + implicit namespace), and `buildContext` returning null when no key is under the caret
